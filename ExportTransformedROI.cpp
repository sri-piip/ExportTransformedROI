...
std::string getImageLocation()
{
	...
	if (SUCCEEDED(hr)) {
		...
		if (SUCCEEDED(hr))
		{
			hr = pFileSave->SetOptions(dwFlags & ~FOS_OVERWRITEPROMPT);

			if (SUCCEEDED(hr))
			{
				pFileSave->SetFileTypes(1, fileTypes);
				pFileSave->SetTitle(L"Select the Destination Image");
...
bool ExportTransformedROI::buildPipeline() 
{
	...
	auto t_imagefilename = getImageLocation();
